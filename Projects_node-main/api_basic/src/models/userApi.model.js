...
class UserApiModel {
  ...
  async loginApiUser(req, res) {
    try {
      // Accept both 'email'/'password' and 'api_user'/'api_password'
      const api_user = req.body.email || req.body.api_user;
      const api_password = req.body.password || req.body.api_password;
      
      console.log("📝 Login attempt - Body received:", req.body);
      console.log("📝 Login attempt - api_user:", api_user);
